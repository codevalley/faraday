...
def test_container_configuration():
    ...
    with patch("src.infrastructure.services.embedding_service.AsyncOpenAI"), \
        ...
        assert isinstance(embedding_service, OpenAIEmbeddingService), f"Expected OpenAIEmbeddingService, got {type(embedding_service)}"
        assert isinstance(vector_store_service, PineconeVectorStore), f"Expected PineconeVectorStore, got {type(vector_store_service)}"
        
        # Verify configuration
        assert embedding_service.api_key == 'test-openai-key', f"Expected 'test-openai-key', got {embedding_service.api_key}"
        assert vector_store_service.api_key == 'test-pinecone-key', f"Expected 'test-pinecone-key', got {vector_store_service.api_key}"
        assert vector_store_service.environment == 'test-env', f"Expected 'test-env', got {vector_store_service.environment}"
...
def test_service_interfaces():
    ...
    assert issubclass(OpenAIEmbeddingService, EmbeddingService), "OpenAIEmbeddingService should inherit from EmbeddingService"
    assert issubclass(PineconeVectorStore, VectorStoreService), "PineconeVectorStore should inherit from VectorStoreService"
    ...
    for method in embedding_methods:
        assert hasattr(EmbeddingService, method), f"EmbeddingService should have {method} method"
        assert hasattr(OpenAIEmbeddingService, method), f"OpenAIEmbeddingService should have {method} method"
    
    for method in vector_store_methods:
        assert hasattr(VectorStoreService, method), f"VectorStoreService should have {method} method"
        assert hasattr(PineconeVectorStore, method), f"PineconeVectorStore should have {method} method"
