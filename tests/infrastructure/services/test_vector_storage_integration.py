...
async def test_index_and_search_thought(embedding_service, vector_store):
    ...
    search_embedding = await embedding_service.generate_embedding("Who did I meet yesterday?")
...
async def test_batch_indexing_and_search(embedding_service, vector_store):
    ...
    # Act - Search for similar vectors
    search_embedding = await embedding_service.generate_embedding("Where did I go in Europe?")
    search_results = await vector_store.search(
        query_vector=search_embedding,
        top_k=5,
