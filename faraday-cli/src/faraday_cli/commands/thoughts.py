...
@click.pass_context
def add(ctx: click.Context, content: str, mood: Optional[str], 
        tags: Optional[str], meta: List[str]) -> None:
    """Add a new thought.
    ...
    if not auth_manager.is_authenticated():
        output.format_error("You must be logged in to add thoughts. Run 'faraday auth login'", 
    ...
    for meta_item in meta:
        if '=' in meta_item:
            ...
            output.format_error(f"Invalid metadata format: {meta_item}. Use key=value format.", 
                              "Validation Error")
            ctx.exit(1)
    ...
    async def do_add():
        try:
            async with api_client:
                thought = await api_client.create_thought(content, metadata if metadata else None)
...
@click.pass_context
def list(ctx: click.Context, limit: int, mood: Optional[str], tags: Optional[str]) -> None:
    ...
    # Check authentication
    if not auth_manager.is_authenticated():
        output.format_error("You must be logged in to list thoughts. Run 'faraday auth login'", 
                          "Authentication Error")
        ctx.exit(1)
    ...
    async def do_list():
        try:
            async with api_client:
                thoughts = await api_client.get_thoughts(limit=limit, filters=filters if filters else None)
...
def show(ctx: click.Context, thought_id: str) -> None:
    ...
    # Check authentication
    if not auth_manager.is_authenticated():
        output.format_error("You must be logged in to view thoughts. Run 'faraday auth login'", 
                          "Authentication Error")
        ctx.exit(1)
...
def delete(ctx: click.Context, thought_id: str) -> None:
    ...
    # Check authentication
    if not auth_manager.is_authenticated():
        output.format_error("You must be logged in to delete thoughts. Run 'faraday auth login'", 
                          "Authentication Error")
        ctx.exit(1)
